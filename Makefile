...
help:
	...
	@echo '  cleanall                clean PLUS remove: DIRS: `build, dist, cover, *._pyxbld` and FILES: `*.so, *.c` and cython annotate html'
...
tests_cover:
	...
	${PYTHON} setup.py build_ext --inplace
	${PYTHON} setup.py clean
	${PYTHON} setup.py nosetests --cover-branches --with-coverage --cover-html --cover-erase --cover-package=TEMPLATE_PyPROJECT
	${PYTHON} setup.py clean
...
code_analysis_pylint:
	pylint -f colorized TEMPLATE_PyPROJECT
